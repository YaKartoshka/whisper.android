...
class MainScreenViewModel(private val application: Application) : ViewModel() {
    ...
    private suspend fun transcribeAudio(file: File) {
        ...
        try {
            ...
            printMessage("Done1 ($elapsed ms): \n$text\n")

            val start1 = System.currentTimeMillis()
            val text1 = whisperContext?.transcribeData(data)

            val elapsed1 = System.currentTimeMillis() - start1
            printMessage("Done2 ($elapsed1 ms): \n$text1\n")

            val start2 = System.currentTimeMillis()
            val text2 = whisperContext?.transcribeData(data)

            val elapsed2 = System.currentTimeMillis() - start2
            printMessage("Done3 ($elapsed2 ms): \n$text2\n")
        } catch (e: Exception) {
            Log.w(LOG_TAG, e)
            printMessage("${e.localizedMessage}\n")
        }
